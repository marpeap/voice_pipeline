...
1. **Le fait le plus contre-intuitif de tout le chantier : un prompt système plus long peut coûter moins cher.** Le cache de prompt a un **préfixe minimum** — **4 096 tokens** sur Claude Haiku 4.5 et sur Gemini Flash. Un prompt de 3 000 tokens n'est donc **pas cachable** et se paie plein tarif à chaque tour, douze fois par appel ; un prompt de 4 200 tokens passe à **0,1×**. **Conséquence de conception** — ⚠️ **révisée le 15/09 par la mesure 23** : le `memoire.md` réel d'un salon fait **854 jetons** comptés par le fournisseur, et il en manque **3 242** pour franchir le seuil, soit cinq fois le fichier. Le « calibrer au-dessus » reviendrait à le **rembourrer** — payer plus cher un texte qui n'apprend rien et qui dilue ce qui compte. **Ce n'est donc pas le fichier du salon qu'on allonge, c'est l'ordre du prompt qu'on inverse** : consignes communes longues et stables d'abord — **identiques pour tous les locataires, donc mises en cache une fois et touchées par toute la flotte** —, `memoire.md` du salon ensuite, variables du tour après la coupure de cache. La doctrine « prompt court » des acteurs du vocal ne s'applique toujours pas, mais la longueur utile est **mutualisée**, pas fabriquée. Mesuré au passage : **3,1 caractères par jeton en français**, et non 4 — toute prévision de coût faite à la louche sous-estime d'un tiers.
...
⚠️ **CORRIGÉ le 14/09 par la mesure 5** : **le tatouage survit intégralement au canal téléphonique** (détection 1,000 après 8 kHz + μ-law, 0,000 sur témoin). **L'argument d'infaisabilité du support est donc mort** — il se déplace sur le **temps réel** : ~1,5 s de tatouage par énoncé sur notre matériel, et **le mode flux d'AudioSeal est cassé en amont** (issue #105). Position plus faible qu'espéré, mais vraie, et appuyée sur une mesure que personne n'a publiée.
