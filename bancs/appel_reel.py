...
def main() -> int:
    ...
    for index, audio in enumerate(entendu):
        ...
        if texte:
            print(f"ce que l'appelant a entendu [{index}] : {texte[:110]}")

    print(f"\nrendez-vous en base : {rendez_vous}")
    if not rendez_vous:
        print("ÉCHEC : aucun rendez-vous n'a été pris.")
        return 1
    print("SUCCÈS : un appel complet, avec les vrais moteurs, a produit un rendez-vous.")
    return 0
